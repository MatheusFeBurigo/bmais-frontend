...
import { ROLE_LABEL, ROLE_DESC, ROLES_ORDEM } from '../lib/usuarioRoles'
...
export default function UsuarioForm() {
  ...
  async function salvar() {
    ...
    setSaving(true)
    try {
      // Só o Analista tem escopo por hospital; os demais veem tudo (envia vazio
      // para não deixar uma restrição "invisível" gravada).
      const escopo = role === 'analista' ? hospitaisSel : []
      if (editando) {
        await atualizarUsuario(id!, { role, hospitais: escopo })
      } else {
        await criarUsuario(email, password, role, escopo)
      }
      ...
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        <div className="card" style={{ padding: 20 }}>
          <div style={{ display: 'grid', gap: 16 }}>
            ...
            {/* Escopo de dados — só para Analista (gestor/diretor/admin veem tudo). */}
            {role === 'analista' && (
              <div>
